...
### Sustituimos el Partido que Genera Conflicto:
```{r}
datos_2015 <- datos_2015 %>%
  mutate(Partido = str_replace_all(Partido, "NVA_ALIANZA", "NUAL"))
```

### Calculamos los Votos por Coalición:
```{r}
datos_2015 <- calcular_votos_coalicion(datos_2015)
```
...
### Filtramos para Comprobar:
```{r}
datos_2015 %>% 
  filter(ID_ESTADO == 31,
         ID_DISTRITO == 2,
         CABECERA_DISTRITAL == "MERIDA") %>% 
  select(Partido, Votos, votos_coalicion)
```
...
### Calculamos los Votos por Coalición:
```{r}
datos_2016 <- calcular_votos_coalicion(datos_2016)
```
...
### Filtramos para Comprobar:
```{r}
print(n=30, datos_2016 %>% 
  filter(ID_ESTADO == 32,
         ID_DISTRITO == 7,
         CABECERA_DISTRITAL == "FRESNILLO") %>% 
  select(Partido, Votos, votos_coalicion))
```
...
### Calculamos los Votos por Coalición:
```{r}
datos_2017 <- calcular_votos_coalicion(datos_2017)
```
...
### Filtramos para Comprobar:
```{r}
print(n=30, datos_2017 %>% 
  filter(ID_ESTADO == 5,
         ID_DISTRITO == 2,
         CABECERA_DISTRITAL == "PIEDRAS NEGRAS") %>% 
  select(Partido, Votos, votos_coalicion))
```
...
datos_2018 <- datos_2018 %>%
  mutate(Partido = str_replace_all(Partido, "_NA", "_NUAL"))

datos_2018 <- datos_2018 %>%
  mutate(Partido = str_replace_all(Partido, "_NA_", "_NUAL_"))

datos_2018 <- datos_2018 %>%
  mutate(Partido = str_replace_all(Partido, "_NA_", "_NUAL_"))

datos_2018$Partido[datos_2018$Partido == "NA"] <- "NUAL"

```

### Calculamos los Votos por Coalición:
```{r}
datos_2018 <- calcular_votos_coalicion_2018(datos_2018)
```
...
### Filtramos para Comprobar:
```{r}
print(n=20, datos_2018 %>% 
  filter(ID_ESTADO == 32,
         ID_DISTRITO_LOCAL == 1,
         CABECERA_DISTRITAL_LOCAL == "ZACATECAS") %>% 
  select(Partido, Votos, votos_coalicion))
```
...
datos_2018 <- datos_2019 %>%
  mutate(Partido = str_replace_all(Partido, "_NA_", "_NUAL_"))

datos_2019 <- datos_2019 %>%
  mutate(Partido = str_replace_all(Partido, "_NA_", "_NUAL_"))

datos_2019$Partido[datos_2019$Partido == "NA"] <- "NUAL"

```

### Calculamos los Votos por Coalición:
```{r}
datos_2019 <- calcular_votos_coalicion_2018(datos_2019)
```
...
### Filtramos para Comprobar:
```{r}
print(n=23, datos_2019 %>% 
  filter(ID_ESTADO == 23,
         ID_DISTRITO_LOCAL == 2,
         CABECERA_DISTRITAL_LOCAL == "CANCÚN") %>% 
  select(Partido, Votos, votos_coalicion))
```
...
```{r}
archivos_csv_2020 <- list.files(path = "~/limpieza/congresos/{{cookiecutter.project_slug}}/data/raw/2020", pattern = "\\.csv$", full.names = TRUE)
```
...
```{r}
datos_2020 <- purrr::map_dfr(archivos_csv_2020, limpiar_datos_2018)
```
...
datos_2020$Partido[datos_2020$Partido == "NA"] <- "NUAL"

```

### Calculamos los Votos por Coalición:
```{r}
datos_2020 <- calcular_votos_coalicion_2018(datos_2020)
```
...
### Filtramos para Comprobar:
```{r}
print(n=23, datos_2020 %>% 
  filter(ID_ESTADO == 5,
         ID_DISTRITO_LOCAL == 5,
         CABECERA_DISTRITAL_LOCAL == "MONCLOVA") %>% 
  select(Partido, Votos, votos_coalicion))
```
...
```{r}
# write_csv(datos_2020, "~/limpieza/congresos/{{cookiecutter.project_slug}}/data/preprocessed/datos_2020.csv")
```
...
### Calculamos los Votos por Coalición:
```{r}
datos_2021 <- calcular_votos_coalicion_2018(datos_2021)
```
...
### Filtramos para Comprobar:
```{r}
print(n=23, datos_2021 %>% 
  filter(ID_ESTADO == 31,
         ID_DISTRITO_LOCAL == 1,
         CABECERA_DISTRITAL_LOCAL == "MERIDA") %>% 
  select(Partido, Votos, votos_coalicion))
```
...
### Calculamos los Votos por Coalición:
```{r}
datos_2022 <- calcular_votos_coalicion_2018(datos_2022)
```
...
### Filtramos para Comprobar:
```{r}
print(n=23, datos_2022 %>% 
  filter(ID_ESTADO == 23,
         ID_DISTRITO_LOCAL == 1,
         CABECERA_DISTRITAL_LOCAL == "KANTUNILKIN") %>% 
  select(Partido, Votos, votos_coalicion))
```
...
### Calculamos los Votos por Coalición:
```{r}
datos_2023 <- calcular_votos_coalicion_2018(datos_2023)
```
...
### Filtramos para Comprobar:
```{r}
print(n=23, datos_2023 %>% 
  filter(ID_ESTADO == 5,
         ID_DISTRITO_LOCAL == 10,
         CABECERA_DISTRITAL_LOCAL == "TORREON") %>% 
  select(Partido, Votos, votos_coalicion))
```
